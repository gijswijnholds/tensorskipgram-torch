...
def evaluate_transitive_models() -> None:
    ...
    result_dict[gs2011.name + '-late'] = {m.name: evaluate_model_on_task_late_fusion(m1, m2, gs2011)
                                          for (m1, m2) in tqdm(trans_models_late)}
    result_dict[ks2013.name + '-early-mid'] = {m.name: evaluate_model_on_task(m, ks2013)
                                               for m in tqdm(trans_models)}
    result_dict[ks2013.name + '-late'] = {m.name: evaluate_model_on_task_late_fusion(m1, m2, ks2013)
                                          for (m1, m2) in tqdm(trans_models_late)}
    result_dict[ks2014.name + '-early-mid'] = {m.name: evaluate_model_on_task(m, ks2014)
                                               for m in tqdm(trans_models)}
    result_dict[ks2014.name + '-late'] = {m.name: evaluate_model_on_task_last_fusion(m1, m2, ks2014)
                                          for (m1, m2) in tqdm(trans_models_late)}
    return result_dict
...
def evaluate_ellipsis_models() -> None:
    ...
    ell_models = ell_models_mid + ell_models_late
    result_dict = {}
...
def get_max_results(result_dict: Dict) -> Dict:
    return {k: round(result_dict[k][max(result_dict[k], key=lambda d: result_dict[k][d][0])][0], 2) for k in result_dict}
